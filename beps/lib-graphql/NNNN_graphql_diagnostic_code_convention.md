...
- **Allocating the federation BEP's codes.** `GRAPHQL_1207`+ and `GRAPHQL_2203`+ are reserved for the [Apollo Federation v2 Parity BEP](NNNN_graphql_federation_v2_parity.md), which defines those diagnostics itself. It currently allocates `GRAPHQL_1207` and `GRAPHQL_1208` for errors and `GRAPHQL_2203` and `GRAPHQL_2204` for warnings, recorded there rather than here.
...
| —             | `GRAPHQL_1012` | **New**, both approaches: `DUPLICATE_FIELD_DECLARATION` — allocated by the [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md) |
| —             | `GRAPHQL_1013` | **New, Approach A only**: `INVALID_REMOTE_METHOD` (remote forbidden entirely; subsumes `GRAPHQL_1001`'s root-level case) — [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md) |
| —             | `GRAPHQL_2001` | **New, Approach B only**: `DEPRECATED_REMOTE_METHOD` (warning) — [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md)   |
| —             | `GRAPHQL_2002` | **New, Approach B only**: `DEPRECATED_GET_ACCESSOR` (warning, two message variants) — [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md) |
...
| `GRAPHQL_119`                                    | `GRAPHQL_1112` | `INVALID_FILE_UPLOAD_IN_RESOURCE_FUNCTION` (re-specified by the [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md)) |
...
| `GRAPHQL_137` | `GRAPHQL_1205` | `FAILED_TO_ADD_ENTITY_RESOLVER` — **removed on delivery** of engine-native `_entities` ([GraphQL Compile-Time Dispatch BEP](NNNN_graphql_compile_time_dispatch.md)) |
...
> **Note:** `GRAPHQL_1207`+ and `GRAPHQL_2203`+ are reserved for the [Apollo Federation v2 Parity BEP](NNNN_graphql_federation_v2_parity.md), which allocates its own codes from that range.
...
| `GRAPHQL_145` | `GRAPHQL_1305` | `INVALID_USAGE_OF_PREFETCH_METHOD_NAME_CONFIG` (message re-worded by the [GraphQL Data Loader Streamlining BEP](NNNN_graphql_dataloader_streamlining.md)) |
| —             | `GRAPHQL_1306` | **New**: `UNDECLARED_DATA_LOADER` (exact trigger depends on which registration path the [GraphQL Data Loader Streamlining BEP](NNNN_graphql_dataloader_streamlining.md) resolves to) |
| —             | `GRAPHQL_1307` | **New**: `INVALID_DATA_LOADER_LOAD_IN_MUTATION` — single-call `load()`/`loadMany()`, [GraphQL Data Loader Streamlining BEP](NNNN_graphql_dataloader_streamlining.md) |
...
| Current       | New            | Name                                                                                                                                                    |
| ------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GRAPHQL_201` | `GRAPHQL_2601` | `UNSUPPORTED_INPUT_FIELD_DEPRECATION` — **removed on delivery** of `@deprecated` on input fields ([GraphQL September 2025 Spec Alignment BEP](NNNN_graphql_september_2025_spec_alignment.md)) |
| —             | `GRAPHQL_1601` | **New**: `INVALID_ONEOF_FIELD` — `@oneOf`, [GraphQL September 2025 Spec Alignment BEP](NNNN_graphql_september_2025_spec_alignment.md)                    |
| —             | `GRAPHQL_1602` | **New**: `INVALID_REQUIRED_DEPRECATED_INPUT` — `@deprecated` on arguments and input fields, [GraphQL September 2025 Spec Alignment BEP](NNNN_graphql_september_2025_spec_alignment.md) |
...
The sibling BEPs currently draw from these ranges. The [GraphQL Unified Resolver Model BEP](NNNN_graphql_unified_resolver_model.md) allocates from area 0: `GRAPHQL_1012` under both approaches, `GRAPHQL_1013` under Approach A, `GRAPHQL_2001` and `GRAPHQL_2002` under Approach B. It also re-specifies `GRAPHQL_1112` in area 1 without renumbering it. The [GraphQL Data Loader Streamlining BEP](NNNN_graphql_dataloader_streamlining.md) allocates `GRAPHQL_1306` and `GRAPHQL_1307` from area 3. The [GraphQL September 2025 Spec Alignment BEP](NNNN_graphql_september_2025_spec_alignment.md) allocates `GRAPHQL_1601` and `GRAPHQL_1602` from area 6, and retires `GRAPHQL_2601` on delivery. The [GraphQL Compile-Time Dispatch BEP](NNNN_graphql_compile_time_dispatch.md) allocates none; it retires `GRAPHQL_1205` and `GRAPHQL_1206` from area 2 when engine-native `_entities`/`_service` resolution lands. The [GraphQL Client Schema Validation BEP](NNNN_graphql_client_schema_validation.md) allocates none either, so area `5` stays reserved and empty: client-side validation surfaces runtime error values rather than compile-time diagnostics. Area 2's remaining range is reserved for the Apollo Federation v2 Parity BEP, per the note in [Section 3](#3-full-mapping).
...
- **The [Apollo Federation v2 Parity BEP](NNNN_graphql_federation_v2_parity.md)'s codes**, allocated from `GRAPHQL_1207`+ and `GRAPHQL_2203`+ and recorded in that BEP.
