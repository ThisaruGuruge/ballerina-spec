...
- Unify the three GraphQL operation types onto a single Ballerina construct — the resource method — with the operation type named explicitly by the accessor (`query`, `mutate`, `subscribe`), and decide, informed by [§1.4](#14-comparison-the-decision-this-proposal-does-not-make), how `get` and `remote` are retired.
...
- **A decision between the two resolver-model approaches.** This proposal designs both the major-version break and the dual-syntax deprecation window in full and compares them unbiased in [§1.4](#14-comparison-the-decision-this-proposal-does-not-make); it does not pick one.
- **Federation v2 parity, designed in full.** Only engine-native `_entities`/`_service` resolution is committed here. The directive set, `FieldSet` validation, reference-resolver narrowing, static composition, and cross-module entities are out of scope for this proposal and belong in a dedicated child BEP — see [§2](#2-federation-high-level-direction-only).
- **The `bal graphql` tool.** Schema-first service generation and client generation are a separate effort. The tool must be updated to emit the new resolver forms; that work is tracked separately. Under the dual-syntax approach this is a soft dependency (old-form generated code still compiles); under the major-version approach it is a hard release dependency — see [§1.4](#14-comparison-the-decision-this-proposal-does-not-make).
...
This section records the present-day behaviour that [Design](#design) changes. Every claim is sourced from the package at `ballerina/graphql` v1.18.0 (distribution `2201.13.3`); `spec.md` references are line numbers in [Ballerina GraphQL Specification](https://github.com/ballerina-platform/module-ballerina-graphql/blob/master/docs/spec/spec.md).

> **Note on `GRAPHQL_nnn` codes.** Every diagnostic code cited in this document under its _current_ number (`GRAPHQL_101` and so on) is emitted by the GraphQL **compiler plugin** — an implementation detail of this package, not a Ballerina language error code. [§0](#0-diagnostic-code-convention) replaces every one of these numbers; this section keeps the current numbers because they're what's actually shipping today, and cross-references the new numbers where useful.
...
The three-way dispatch at the bottom of the runtime path (`RD1`/`RD2`/`RD3`) is the structural consequence of the current resolver model. `Engine.java` selects between them on the document's operation type; `ServiceAnalyzer.java` walks `getResourceMethods()` and `getRemoteMethods()` as two separate loops. Each of `RD1`/`RD2`/`RD3` is, today, a `for` loop over every resource or remote method on the `ServiceType`, run again for every field of every request — see [§1.2](#12-shared-compile-time-resolved-runtime-dispatch) for the exact call sites.

### Resolver model

| GraphQL root type | Ballerina form                                                | Enforced by                                          |
| ----------------- | ------------------------------------------------------------- | ---------------------------------------------------- |
| `Query`           | `resource function get <path>(...)`                           | `ServiceValidator.validateRootServiceResourceMethod` |
| `Mutation`        | `remote function <name>(...)`                                 | `ServiceValidator.validateRootServiceMethods`        |
| `Subscription`    | `resource function subscribe <name>(...) returns stream<...>` | `ServiceValidator.validateSubscribeResource`         |
...
- Mutations execute serially; queries execute in parallel (spec.md:520). This is keyed off the _document's_ operation type in `executor_visitor.bal`, not off the method kind.

The accessor strings are hardcoded in four places:

```java
// compiler-plugin/.../service/validator/ValidatorUtils.java:46-47
public static final String RESOURCE_FUNCTION_GET = "get";
public static final String RESOURCE_FUNCTION_SUBSCRIBE = "subscribe";

// native/.../engine/EngineUtils.java:101-102
...
// Engine.java:166-174 — called once per Mutation field, per request
for (RemoteMethodType remoteMethod : serviceType.getRemoteMethods()) {
    if (remoteMethod.getName().equals(fieldName)) { /* ... */ }
}

// Engine.java:117-124 — called once per Subscription field, per request
for (ResourceMethodType resourceMethod : serviceType.getResourceMethods()) {
    if (SUBSCRIBE_ACCESSOR.equals(resourceMethod.getAccessor()) && /* path match */) { /* ... */ }
}
```

`getResourceAnnotation` (`Engine.java:253-269`) redoes the same scan again, separately, to look up `@graphql:ResourceConfig` for the same field — so a single field resolution can trigger this linear scan more than once. None of this is cached per listener; it's redone on every request.

The package already has the right piece of infrastructure to fix this without adding a new one: `EngineUtils.analyzeServices` runs a `ServiceAnalyzer` once per service, at listener attach time, which walks `getResourceMethods()`/`getRemoteMethods()` exactly once and stores a `Map<String, Resource>` keyed by schema coordinate (e.g. `"Query.greeting"`) as native data on the service object (`RESOURCE_MAP`, `ServiceAnalyzer.java:63,169`). Today, the `Resource` record this map holds carries only a `complexity` value (used by `getFieldComplexity`) — not a reference to the method itself. See [§1.2](#12-shared-compile-time-resolved-runtime-dispatch).

### Federation / subgraph

`ballerina/modules/subgraph/` is 90 lines of Ballerina across three files and contains declarations only — no logic.

Supported Federation directives, from the closed enum `commons/.../types/FederatedDirective.java:33-34`, are **exactly two**:
...
The `@link` directive it emits already points at `https://specs.apollo.dev/federation/v2.0` (`SdlSchemaStringGenerator.java:83`), so the package is not stuck on the Federation v1 specification — what it lacks is coverage of the rest of the v2 directive set. The `_entities`/`_service` resolvers are injected as Ballerina source at compile time by `GraphqlSourceModifier.getEntityResolver` (`:342-390`), which reads `.bal.partial` template files and performs `replaceAll` substitution. Failure surfaces as `GRAPHQL_137`/`GRAPHQL_138`. Full design work addressing these limitations is deferred to a dedicated child BEP rather than designed here — see [§2](#2-federation-high-level-direction-only).

### Root type naming

`Query`, `Mutation`, and `Subscription` are not user-authored symbols anywhere in the package. The engine infers them from the service: the root object encountered at the service declaration becomes `Query`, its resource methods with a `mutate` accessor become `Mutation`, and so on (`ServiceAnalyzer.java`, `SchemaGenerator.java`). There is no equivalent of the `schema { query: MyQuery }` root-operation-type-alias construct that the GraphQL specification permits ([GraphQL specification — Type System, Schema](https://spec.graphql.org/September2025/#sec-Schema)). Two concrete consequences: a user cannot give the root types their own names, and a user cannot attach a type-level doc comment to `Query`, `Mutation`, or `Subscription`. This is a narrow gap in practice, and it is recorded here rather than assumed away. It is out of scope for this proposal; see [Non-Goals](#non-goals) and [Future Work](#future-work).

### Data loader

The entire `graphql.dataloader` module is 132 lines in one file:
...
1. **The specification's own examples contradict the implementation on the prefetch method's kind — a documentation deviation, not a functional defect.** The shipped mechanism works correctly and exactly as designed: a prefetch method must be a plain method (`method.kind() == SymbolKind.METHOD && !isRemoteMethod(method)`, `ServiceValidator.java:485-489`), matching spec.md:3748's `function preBooks(graphql:Context ctx)`. The defect is that three _other_ examples in the same document — spec.md:3804, 3857, and 3875 — show `resource function get preBooks(...)` instead, which silently compiles into an unwanted `preBooks` field on `Query` rather than a working prefetch hook. See [§3.2](#32-fix-the-prefetch-method-contract).
2. **`docs/proposals/graphql-dataloader.md` describes an API that was never shipped** — a `@dataloader:Loader { batchFunctions: {...} }` annotation, a `load()` method (shipped as `add()`), and `map<dataloader:DataLoader>` resolver parameters. This document is **not** touched by this proposal — see the note in [Migration Guide, Documentation](#documentation).
...
No parsing or validation happens client-side; the document is sent as-is. `init` pins the underlying client to HTTP/1.1 (`client.bal:31`) — the pin exists because subscriptions (added by BEP 1460) go over a WebSocket upgrade, and Ballerina's WebSocket support requires HTTP/1.1. There is no subscription support in the shipped client today and no file-upload support.

BEP 1460 (now vendored into this repository at
[`1460_graphql_client_subscription_support.md`](1460_graphql_client_subscription_support.md), read in full for this revision — see [§5](#5-client-side-schema-aware-validation)) adds `query()`, `mutate()`, and `subscribe()`; deprecates `execute()` **without removing it** ("It remains functional throughout a deprecation period and will be removed in a later major version" — 1460 §API Changes); and removes the already-deprecated `executeWithType()` along with the `graphql:ServerError` type, which 1460 confirms is used only by `executeWithType()` and constructed nowhere else. This proposal does not restate 1460's design, but does rely on it directly in §5 and in the breaking-change accounting throughout this document.

### GraphQL over HTTP conformance

`listener_utils.bal:42-46` accepts three request content types — `application/json`, `application/graphql`, and `multipart/form-data` — and there is no handling of the `application/graphql-response+json` response media type and no `Accept` header negotiation. The GraphQL over HTTP specification is at Stage 2 (draft) and "may continue to evolve", so alignment is proposed as a discrete, separable item rather than as a conformance requirement.

### Blast radius, by approach

The blast radius of the resolver-model change is entirely different depending on which approach in [§1](#1-unified-resolver-model) is chosen. Under the major-version break, every existing `get`/`remote` declaration across the ecosystem must be rewritten. Under dual-syntax, existing declarations don't need to change at all — only new fixtures for the new forms and the deprecation warnings are added. See the comparison table in [§1.4](#14-comparison-the-decision-this-proposal-does-not-make) for the two blast radii side by side.
...
| 1    | [Unified resolver model](#1-unified-resolver-model)                                      | **Depends on the approach chosen — see §1.4**                                  |
| 2    | [Federation — direction only](#2-federation-high-level-direction-only)                   | No (this proposal); TBD in the child BEP                                       |
| 3    | [Data loader streamlining](#3-data-loader-streamlining)                                  | Partly (`Context.getDataLoader` signature, §3.4)                               |
| 4    | [Directives ratified in the September 2025 edition](#4-directives-and-input-type-system) | Partly (`@deprecated` on a required argument, §4.2)                            |
...
| —             | `GRAPHQL_1012` | **New**, both approaches: `DUPLICATE_FIELD_DECLARATION` — see [§1.2](#12-shared-compile-time-resolved-runtime-dispatch)                         |
...
| `GRAPHQL_119`                                    | `GRAPHQL_1112` | `INVALID_FILE_UPLOAD_IN_RESOURCE_FUNCTION` (re-specified — see [§1.2](#12-shared-compile-time-resolved-runtime-dispatch)) |
...
| `GRAPHQL_137` | `GRAPHQL_1205` | `FAILED_TO_ADD_ENTITY_RESOLVER` — **removed on delivery** of engine-native `_entities` ([§2](#2-federation-high-level-direction-only)) |
...
| `GRAPHQL_145` | `GRAPHQL_1305` | `INVALID_USAGE_OF_PREFETCH_METHOD_NAME_CONFIG` (message re-worded — [§3.2](#32-fix-the-prefetch-method-contract))                                |
...
| Current       | New            | Name                                                                                                                                                    |
| ------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GRAPHQL_201` | `GRAPHQL_2601` | `UNSUPPORTED_INPUT_FIELD_DEPRECATION` — **removed on delivery** of `@deprecated` on input fields ([§4.2](#42-deprecated-on-arguments-and-input-fields)) |
...
Accessors are verbs in Ballerina by convention (`get`, `post`, `put` in `ballerina/http`), and a resource accessor is grammatically an _identifier_ — `resource-method-name := identifier` ([Ballerina language specification, Resources](https://ballerina.io/spec/lang/master/#resources_defn)) — so all three names are legal without any language change. This was an open item in an earlier draft (whether abandoning `get` has any consequence for the language's resource-path _delegation_ semantics, defined only for `get`) and is resolved: the GraphQL engine has never dispatched through that machinery — it uses its own runtime resource-method lookup, validated entirely by the compiler plugin — so the language-level delegation rule is not in this package's dispatch path at all, regardless of which resolver-model approach ships.
...
**The problem, precisely** ([Current State Analysis](#runtime-dispatch-performance)): `Engine.java`
resolves every `Query`/nested-object field, every `Mutation` field, and every `Subscription` field by linearly scanning `ServiceType.getResourceMethods()`/`getRemoteMethods()` — on every single field resolution, on every request. The compiler plugin already knows, per declared path, exactly which method serves which field; that knowledge is discarded once compilation finishes.

**The fix reuses existing infrastructure rather than adding a new one.** `ServiceAnalyzer` already walks every resource and remote method exactly once, at listener-attach time, building a `Map<String, Resource>` keyed by schema coordinate and stashing it as native data on the service object (`RESOURCE_MAP`). Today `Resource` carries only a `complexity` value. This proposal extends `Resource` to also carry the resolved `MethodType` (the `ResourceMethodType` or `RemoteMethodType` reference), so that:

- `Engine.getResourceMethod`/`executeMutationMethod`/`executeSubscriptionResource`/`getResourceAnnotation` all become `resourceMap.get(coordinate)` — a map lookup, not a scan — regardless of which accessor spelling produced that coordinate at compile time.
- Building the map is also the natural place to enforce the one new cross-cutting rule both approaches need: **two declarations that resolve to the same schema coordinate is a compile error.** Today, `ServiceAnalyzer.analyzeResourceMethod`'s `if (this.resourceMap.containsKey(coordinate)) return;` guard is unreachable in practice, because the compiler plugin already prevents two same-spelling declarations of the same field from compiling. Once a service can (under Approach B) or, transiently during a migration, mixed-up code under either approach could declare `resource function get profile()` and `resource function query profile()` on the same type, this exact guard is where a **new compile-time check** (`GRAPHQL_1012`, `DUPLICATE_FIELD_DECLARATION`) belongs: the compiler-plugin-side `ServiceValidator` collects query-type fields (from `get`- and `query`-accessor resource methods together) and mutation-type fields (from `remote` methods and `mutate`-accessor resource methods together) into one namespace per operation type, and rejects a name collision within a namespace — the language does not do this for us, since a `remote function updateName` and a `resource function mutate updateName` are, to the language, two unrelated declarations that happen to share a name.
...
**Why this belongs in this proposal regardless of which resolver-model approach ships:** the fix is a strict improvement to today's already-shipped `get`/`remote`/`subscribe` dispatch, independent of whether a second spelling is ever added. This proposal is already rewriting every call site in `Engine.java` that touches resource/remote method dispatch (Approach A rewrites them to remove a branch; Approach B rewrites them to add one) — doing the dispatch-table fix in the same pass costs little extra and is what makes Approach B's "does dual dispatch cost more on the hot path" question moot rather than merely asserted-away.
...
**Diagnostics** (using the new numbering from [§0](#0-diagnostic-code-convention)):
...
**Testing obligation specific to this approach:** schema-equivalence fixtures captured _before_ the sweep (for every existing test service that doesn't use a hierarchical path, generated SDL after migration must be byte-identical to before); golden diagnostic messages in `compiler-plugin-tests` re-baselined one at a time, never in bulk, so a bulk accept doesn't mask a real regression.
...
**Testing obligation specific to this approach:** every pre-existing test fixture using `get`/`remote` compiles and passes **without modification** — worth asserting directly (e.g. a CI check that fails if this phase's implementation touches a pre-existing fixture file), since it's the primary evidence the change is genuinely additive. New fixtures cover `query`/`mutate`, the two deprecation warnings (in both message variants for `GRAPHQL_2002`), a mixed-form service, and the new `GRAPHQL_1012` duplicate-field collision across accessor families.
...
| **Ecosystem/version-resolution consideration**                                      | A user pinned to the old major version in their own `Dependencies.toml` continues to work indefinitely on that version; new projects start on the new one. **Reported claim, needs verification before this proposal is finalized:** Ballerina's package resolution does not permit two different major versions of the _same_ package to be used simultaneously within one project's dependency graph. For most packages (e.g. `ballerina/http`, which many other connectors depend on transitively) this makes a hard break costly, because one project can get stuck between two transitive dependents that require different majors. For `ballerina/graphql` specifically, this risk is narrow: no other Ballerina Central packages or connectors are known to depend on `graphql` transitively (unlike `http`), so the conflict mainly arises only if a _user_ has published their own reusable library on top of `graphql` v1 that other users also import — a real but narrow case. This claim should be verified against Ballerina's current package-resolution algorithm before it's relied on as a decision input. | Not applicable in the same way — there is only ever one major version in play, because nothing is removed.                                                                                                                                                                                                                                                                                                                                                      |
...
Both approaches keep [§1.1](#11-shared-the-three-accessors) and [§1.2](#12-shared-compile-time-resolved-runtime-dispatch) identical; the table above is the complete set of differences. Neither approach is presented here as the recommendation of this proposal.
...
**The one piece of direction this proposal does commit to**, because it's foundational to [§1.2](#12-shared-compile-time-resolved-runtime-dispatch) and shouldn't wait on the rest of federation's design: `_entities` and `_service` are resolved **natively by the engine**, not injected as compiled Ballerina source. Today, `GraphqlSourceModifier.getEntityResolver` reads `entity_resolver.bal.partial` and `service_resolver.bal.partial`, performs `replaceAll` placeholder substitution, and parses the result with `NodeParser.parseObjectMember` — a compile-time templating step that can fail (`GRAPHQL_1205`/`GRAPHQL_1206`) independently of anything the user wrote. Once the engine holds a compile-time-resolved dispatch table for ordinary fields anyway, the same structure holds an entity type map (`__typename` → type symbol + `resolveReference`) with no need to synthesise Ballerina source at all: `_service { sdl }` is answered directly from `schemaString`, and `_entities(representations:)` dispatches on `__typename` through the map. `GRAPHQL_1205`/`GRAPHQL_1206` are removed once this ships, since the failure modes they report cannot occur once there's no source injection to fail.
...
The design goal is to remove boilerplate that carries no information, and to make what remains compile-time checked. Three changes, in increasing order of risk. This part is independent of the decision in [§1](#1-unified-resolver-model) and unaffected by which approach is chosen there.
...
**Breaking-change consideration.** This field addition, by itself, is additive to a Ballerina program's source. It is worth flagging alongside a related, independently-discovered nuance in §3.4 below and in [BEP 1460](https://github.com/ballerina-platform/ballerina-spec/issues/1460)'s own risk register: 1460 notes that adding a field to a _closed_ record (`record {| ... |}`) "breaks binary (BIR) compatibility for dependents compiled against the previous version" — `GraphqlServiceConfig` is declared as a closed record today. This proposal's field additions here and in §5 share that same property. See [Risks](#risks) for the consolidated accounting rather than repeating it per field.

#### 3.2 Fix the prefetch method contract

The prefetch method is a plain method, not a resource method; this is correct today and unaffected by either resolver-model approach. The specification's examples must be corrected at spec.md:3804, 3857, and 3875, and `compiler-plugin-tests` must gain a negative fixture asserting that `resource function query preBooks(...)` (and, under Approach B, `resource function get preBooks(...)`) is exposed as a `Query` field named `preBooks` and is _not_ treated as a prefetch hook.
...
- **Infeasible without language or runtime support** — a language-level requirement is raised, and the two-phase form remains the supported API with §3.1 and §3.2 as the shipped improvements.
...
Independent of §3.3, the `DataLoader` object type gains operations every comparable implementation provides and this one lacks:
...
This proposal ships it as designed — breaking, now — on the basis that the migration cost is small and mechanically caught by the compiler, and that panic-to-checked-error is a strictly better contract worth taking regardless of how [§1](#1-unified-resolver-model) resolves. Reviewers who weigh this differently should treat it as a separable decision from §1's.

> **Note on generics.** A type-parameterised `DataLoader<K, V>` would eliminate the remaining `anydata` looseness, but Ballerina has no user-definable generic object types. The dependently-typed `typedesc` parameter on `get`/`getMany` is the closest available approximation and is retained.

### 4. Directives and input type system

This part is independent of [§1](#1-unified-resolver-model) and unaffected by which approach ships there.
...
This also gives Ballerina a usable input-union idiom for the first time. The existing restriction that input unions may only be `T|()` (`GRAPHQL_1115`) is unchanged; `@graphql:OneOf` is the supported way to express "one of several inputs". spec.md:980 ("Input Unions") is updated to point at `@oneOf`.
...
**This is a narrow, new source of compile breaks, worth naming explicitly rather than folding silently into "Part 4 is mostly additive":** a service that carries Ballerina's `@deprecated` annotation on a _required_ resolver argument or input field compiles today (`GRAPHQL_2601` is only a warning). Under this change it becomes a compile error, since a required deprecated input is a contradiction the specification itself doesn't allow. Expected to be rare — deprecating a required input is unusual practice — but it is real and independent of anything in [§1](#1-unified-resolver-model).
...
This part builds directly on [BEP 1460](1460_graphql_client_subscription_support.md), now read in full and vendored into this directory rather than described second-hand. 1460 introduces `query()`, `mutate()`, and `subscribe()` and always performs syntax-level validation plus an operation-kind match, using the packaged `graphql.parser`. Executing a mismatched document returns `graphql:InvalidDocumentError` without a network call. On its own timeline, 1460 also deprecates `execute()` (kept working, "removed in a later major version" per 1460's own text — the same deprecate-without-removing shape as this proposal's Approach B in §1, arrived at independently) and removes the already-deprecated `executeWithType()` together with the `graphql:ServerError` type, which 1460 confirms is used only by `executeWithType()` and constructed nowhere else in the package.
...
- `SCHEMA` reuses the listener-side validator visitors (`field_validator_visitor.bal`, `variable_validator_visitor.bal`, `directive_validator_visitor.bal`, `fragment_validator_visitor.bal`) against a `__Schema` value built from the configured SDL. The validation logic is therefore shared, not duplicated.
...
> **Implementation dependency.** The package contains an SDL _generator_ (`SdlSchemaStringGenerator`, 532 LOC in `commons`) but **no SDL parser**. `SCHEMA`-level validation requires one. This is the largest single new component in this proposal and is why the tier is opt-in rather than the default. An SDL parser is independently valuable: it is also the prerequisite for schema-first service generation in the `bal graphql` tool and for federation composition checks.
...
**Breaking-change accounting for this part, made explicit rather than left implicit:** adding `documentValidation` to `ClientConfiguration` shares the same closed-record BIR-compatibility property BEP 1460 already flags for its own additions to the same record (`subscription` field) — see the note in [§3.1](#31-declarative-loader-registration) and the consolidated discussion in [Risks](#risks). Beyond that, this part of the proposal introduces no new breaking change of its own; every breaking change in the client's API surface (`executeWithType()`/`ServerError` removal) is BEP 1460's, on 1460's own timeline, independent of this proposal.

### API Reference

Consolidated view of the public surface after this proposal, **branched where [§1](#1-unified-resolver-model)'s two approaches differ**.
...
Unchanged in shape. `prefetchMethodName` remains valid on every resolver form that maps to `Query` or `Mutation` (which form that is depends on the approach chosen in §1) and invalid on `subscribe` resource methods.
...
public type DataLoader isolated object {
    ...
    //   ^ subject to the outcome of the spike in §3.3
};
```
...
**This section branches entirely on which approach [§1](#1-unified-resolver-model) resolves to.**
...
4. **`Context.getDataLoader` call sites.** Add `check` — see [§3.4](#34-convenience-additions).
5. **`@deprecated` on a required argument or input field.** Fails to compile — see [§4.2](#42-deprecated-on-arguments-and-input-fields).

**Tooling.** A migration tool is a release requirement for this approach, not a nice-to-have — see [§1.3](#approach-a--major-version-hard-break).
...
**Tooling.** Not required for this release — see [§1.3](#approach-b--dual-syntax-deprecation-window). Track building one as [Future Work](#future-work), triggered by whichever future release removes `get`/`remote`.
...
`docs/spec/spec.md`, `README.md`, `examples/`, Ballerina by Example, and published guide content (e.g. [`graphql-guide`](https://github.com/ThisaruGuruge/graphql-guide)) must show the **new, recommended forms only** — `query`/`mutate`, never `get`/`remote` — regardless of which approach in §1 ships. New users reading these should see the idiom this proposal recommends, not the one it's retiring or deprecating; under Approach B specifically, continuing to teach the old forms in fresh material would actively work against the point of deprecating them. This applies with equal force under both approaches, and matters _more_ under Approach B, where a compile error isn't there to catch a regenerated old-form example (see [§1.4](#14-comparison-the-decision-this-proposal-does-not-make)'s AI-assistant-risk row).

**`docs/proposals/` (the 14 existing documents in the module repository) are explicitly out of scope for this sweep and must not be edited.** These are added when a proposal is implemented and are kept unmodified afterward as a historical record of what was accepted and shipped at the time — including `graphql-dataloader.md`, which describes an API that was never actually shipped (see [Current State Analysis, Data loader](#data-loader)). That inaccuracy is a pre-existing fact about the historical record, not something this proposal corrects.

The following documentation defects, unrelated to either resolver-model approach, are fixed in the same pass regardless of which one ships:

| Location                                   | Defect                                                                                                   |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| spec.md:3804, 3857, 3875                   | prefetch methods shown as `resource function get preX`; must be plain methods                            |
| spec.md:1764 vs `types.bal:64`             | GraphiQL default `path` documented as `"/graphiql"`, implemented as `"graphiql"`                         |
| `GRAPHQL_1005` message (was `GRAPHQL_113`) | references `@dataloader:Loader`, which does not exist in the shipped module                              |
| spec.md:980                                | records "Input Unions" as a gap; should point at `@oneOf`                                                |
| spec.md:3420, 3620                         | federation composition/reference-resolver notes — update once the federation child BEP lands, not before |

## Alternatives

### Dual-accessor support during a deprecation window

**No longer an alternative — this is [§1.3](#approach-b--dual-syntax-deprecation-window)'s Approach B, one of the two options this proposal presents in full.** An earlier draft of this proposal rejected this outright, on two grounds: that the runtime's accessor-keyed dispatch would have to try each accepted accessor in turn on the hot path, and that dual support leaves the semantics of a mixed-form service undefined. Recorded here, rather than silently dropped, because a BEP is a decision record and a reader who finds the earlier committed rejection in this document's history deserves the explanation for the reversal: [§1.2](#12-shared-compile-time-resolved-runtime-dispatch) resolves the first ground directly (a compile-time-resolved dispatch table makes accessor spelling irrelevant to per-request dispatch cost), and [§1.3](#approach-b--dual-syntax-deprecation-window) resolves the second by specifying the mixed-form semantics explicitly (free per-field mixing, a new compile-time duplicate-field check for a same-coordinate collision across accessor families) rather than leaving them open. With both objections answered, the option was promoted from a rejected alternative to one of the two designs presented for a decision.

### Keeping the current resolver forms and shipping only the additive parts

A genuinely non-destructive alternative: leave `get`/`remote`/`subscribe` exactly as they are, and ship only [Part 0](#0-diagnostic-code-convention), the federation direction in [Part 2](#2-federation-high-level-direction-only), [Part 3](#3-data-loader-streamlining), [Part 4](#4-directives-and-input-type-system), and [Part 5](#5-client-side-schema-aware-validation) — every one of which is independent of §1's decision anyway.
...
**Rejected for scope**, per [§4.3](#43-custom-scalars-and-specifiedby-direction-only).
...
Testing obligations specific to each approach are stated in their respective subsections of [§1.3](#13-the-open-decision). Shared across both:

- **Compile-time-resolved dispatch table.** Unit tests asserting `ServiceAnalyzer`'s extended `Resource` record carries the correct method reference for every field, including nested-object and federation entity fields; a regression test that `Engine.getResourceMethod`/`executeMutationMethod`/ `executeSubscriptionResource`/`getResourceAnnotation` no longer scan `getResourceMethods()`/ `getRemoteMethods()` per call (e.g. an instrumented test double, or an assertion on call counts against the underlying `ServiceType` accessor methods).
- **`GRAPHQL_1012` (`DUPLICATE_FIELD_DECLARATION`).** A `get`/`query` collision on the same path (and, on Approach B, `remote`/`mutate` on the same name) is rejected.
- **Runtime dispatch tests.** Mutations remain serial and queries parallel — keyed off the document's operation type, unaffected by anything in this proposal; existing `executor_visitor` tests must pass unchanged.
- **Prefetch-method negative test**, per [§3.2](#32-fix-the-prefetch-method-contract).

### Diagnostic code convention

- Every renumbered code in [§0.3](#03-full-mapping) gets a fixture asserting the **new** number and message; a repository-wide grep confirms no reference to a pre-renumbering code string survives outside historical/changelog text.
- A test asserting every constant in the regenerated `DiagnosticCode` enum follows the `GRAPHQL_nnnn` pattern (guards against reintroducing the `WARNING_209`/`WARNING_210`-style naming bug).

### Federation

Deferred to the child BEP. The one committed item, engine-native `_entities`/`_service`, needs parity tests against the current source-injected behaviour, including every error path and its exact message text.

### Data loader

- `dataLoaders` registration: one loader per entry per request, not shared across requests; `contextInit` continues to work; the explicit **collision case** (a `dataLoaders` entry and a `contextInit` registration using the same name) asserting `contextInit` wins.
- `GRAPHQL_1306`: undeclared literal key (error); non-constant key (warning); key declared via `contextInit` in the same module (no diagnostic).
- `getDataLoader` returning `Error` instead of panicking.
- `addMany`, `getMany`, `prime`, `clear` unit tests, including `getMany` result ordering and `prime` interaction with `dispatch`.
- If the [§3.3](#33-single-phase-load-spike-required-before-commitment) spike succeeds: concurrency and deadlock tests for `load()`.

### Directives and input types

- `@oneOf`: schema output; compile errors for non-optional/non-nilable/defaulted fields; coercion errors for zero, two, and exactly-one-with-explicit-null field cases; introspection `isOneOf`.
- `@deprecated` on a resolver parameter and an input-object field: schema output; `reason` from the documentation section; compile error for a required deprecated input.

### Client

- Validation levels: `NONE` sends anything; `SYNTAX` behaves as BEP 1460 specifies; `SCHEMA` catches unknown fields/arguments/argument-type mismatches/invalid variable usage/invalid fragment conditions **without a network call**.
- Equivalence test: `SCHEMA`-level client validation's `ErrorDetail[]` matches what the listener produces for the same document against the same schema.
- SDL parser round-trip suite: every SDL construct the generator can emit must round-trip through the parser.

### Migration tool (Approach A only)

If Approach A is chosen, the migration tool needs its own coverage: corpus coverage against the full `ballerina-tests`/`compiler-plugin-tests` fixture set; hierarchical-path sites flagged, not silently rewritten; idempotency on already-migrated input; correct handling of partially-migrated input.

## Risks and Assumptions

### Risks

- **The choice between Approach A and Approach B carries genuinely different risk profiles — see the comparison table in [§1.4](#14-comparison-the-decision-this-proposal-does-not-make) rather than a restatement here.** The single highest-leverage open risk in this entire proposal is that this decision hasn't been made yet.
...
- **Closed-record field additions have a BIR-compatibility cost independent of source compatibility.** `GraphqlServiceConfig`, `GraphqlResourceConfig`, and `ClientConfiguration` are all closed records (`record {| ... |}`). BEP 1460 already flags this for its own additions to `ClientConfiguration` ("breaks binary (BIR) compatibility for dependents compiled against the previous version"); this proposal's additions (`dataLoaders`, `documentValidation`) share the identical property. This is a narrower, real cost distinct from source-level breaking changes, and is not mitigated by choosing Approach B for §1 — it applies regardless.
- **`GRAPHQL_1xxx`/`2xxx`/`3xxx` code allocation** (§0.3) is proposed, not final; the exact numbers must be confirmed against whatever else has landed in the package by implementation time.
- **The §3.3 spike may fail.** If single-phase `load()` proves infeasible without runtime support, the data loader improvement is limited to declarative registration and compile-time key checking.
- **The SDL parser is a new component with no existing counterpart in the package** — the schedule risk in §5, mitigated by `SCHEMA` being opt-in.
...
- **`query`, `mutate`, and `subscribe` are usable as resource accessors with no language change.** Confirmed — see [§1.1](#11-shared-the-three-accessors).
...
- **The `ArgumentHandler` accepts a `ResourceMethodType` where it currently receives a `RemoteMethodType` for mutations (relevant only if `mutate` is added, i.e. under either approach).** Not yet verified.
- **Serial mutation execution is unaffected**, because it keys off the document's operation type, not the Ballerina method kind. Verified in `executor_visitor.bal`.
- **BEP 1460 is accepted and lands in the same or an earlier release.** No longer a second-hand assumption — 1460 has been read in full for this revision (see [§5](#5-client-side-schema-aware-validation)) and is vendored into this directory. If 1460 is substantially revised after this point, §5 must be revisited.
...
- **The GraphQL over HTTP draft is a moving target.** [§4.4](#44-graphql-over-http-alignment) is scoped as alignment with a Stage 2 draft, not conformance to a ratified specification.
- **Ballerina's package-resolution behaviour regarding two major versions of one package** is stated as a **reported claim requiring verification** in [§1.4](#14-comparison-the-decision-this-proposal-does-not-make)'s comparison table, not asserted as confirmed fact. Verify before using it as a decision input.
- **No performance regression is expected** from either resolver-model approach; the dispatch-table change in [§1.2](#12-shared-compile-time-resolved-runtime-dispatch) is expected to be a strict improvement over today. Not yet measured.

## Dependencies

- **[BEP 1460 — GraphQL Client Subscription Support](https://github.com/ballerina-platform/ballerina-spec/issues/1460).**
  Vendored into this directory and read in full for this revision. Supplies the `query()`/`mutate()`/ `subscribe()` client methods and the `SYNTAX` validation tier [§5](#5-client-side-schema-aware-validation) builds on, and independently removes `executeWithType()`/`ServerError` on its own timeline.
...
- **A decision on [§1](#1-unified-resolver-model)**, and, if Approach B is chosen: a target release for eventually removing `get`/`remote`, and a migration tool built ahead of that release rather than left indefinitely deferred.
- **Federation v2 parity**, as a dedicated BEP.
- **Custom scalars** — the full API, per the direction in [§4.3](#43-custom-scalars-and-specifiedby-direction-only). Unblocks `@specifiedBy`, a `JSON` scalar, and domain scalars such as `DateTime` and `URL`.
...
- **`records.bal` cleanup** — 14 near-empty `*http:X` inclusion wrappers that exist only to re-export HTTP configuration types. A natural candidate for a breaking release.
...
> **Resolved, not deferred.** Under Approach A, hierarchical-path result assembly's two long-standing TODOs (`common_utils.bal:495`, `response_generator.bal:77`) are removed outright as a side effect of removing hierarchical paths. Under Approach B, they remain live, tracked against whichever future release eventually removes `get`.
